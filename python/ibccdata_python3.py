...
#                 logging.error('Could not save the input data as a Python object file.')
    ...
    def loadData(self, configFile):
        ...
        #read configuration
        print(('Will now try to read configuration from %s' % configFile))
        ...
        if 'outputFile' in namespace:
            outputFile = namespace['outputFile']
        if 'confMatFile' in namespace:
            confMatFile = namespace['confMatFile']
        if 'hyperparam_file' in namespace:
            hyperparam_file = namespace['hyperparam_file']
        if 'inputFile' in namespace:
            inputFile = namespace['inputFile']
        if 'goldFile' in namespace:
            goldFile = namespace['goldFile']
        if 'scores' in namespace:
            scores = namespace['scores']
        if 'nClasses' in namespace:
            nClasses = namespace['nClasses']
        if 'nu0' in namespace:
            nu0 = namespace['nu0']
        if 'alpha0' in namespace:
            alpha0 = namespace['alpha0']
        if 'phi0' in namespace:
            phi0 = namespace['phi0']
        if 'gamma0' in namespace:
            gamma0 = namespace['gamma0']
        if 'a0' in namespace:
            a0 = namespace['a0']
        if 'b0' in namespace:
            b0 = namespace['b0']
