...
class DynamicConfusionMatrix(ibcc.ConfusionMatrix):
    N = 0
    Tau = 0
    verbose = False
    alpha0_tau = None # the hyper-parameters copied out for each time-step. alpha0 has only one matrix for each agent.    
    table_format_flag = False
    
    def set_data_format(self, table_format_flag):
        if table_format_flag:
        ...
        self.table_format_flag = table_format_flag
    
    def init_lnPi(self, K, Tau, Cknown):
        '''
...
# Posterior Updates to Hyper-parameters -----------------------------------------------------------------------------
    def _post_Alpha(self, E_t, trainidxs, testidxs, C, Ctest, Ntrain, Ntest):#Posterior update to hyper-parameters
        self.N = Ntrain * Ntest
         
        if self.nclasses>2:
            for l in range(self.nscores):
                self.post_Alpha_binary(l, E_t, trainidxs, testidxs, C, Ctest, Ntrain, Ntest, l)
        else:
            self.post_Alpha_binary(E_t, trainidxs, testidxs, C, Ctest, Ntrain, Ntest)

    def h_cov(self, n, E_t):
        h = E_t[n, :].reshape((1,1,self.nclasses))
            # the expanded vector to use with block-diagonal covariance
        return block_diag(*np.tile(h, (self.K, 1, 1))).T  # nclasses*K x K):
               
    def post_Alpha_binary_sparselist(self, E_t, trainidxs, testidxs, C, Ctest, Ntrain, Ntest, l=1):
        ...
        while tau < self.Tau:
            ...
            tau_pr = tau_prev[k]
            tau_prev[k] = tau
            h = E_t[i, :].reshape((self.nclasses, 1))  # column vector
            if tau_pr == -1:
        ...
        while tau > 0:
            ...
            k = self.Cagents[tau]
            i = self.Cobjects[tau]
            h = E_t[i, :].reshape((self.nclasses, 1))
    ...
    def post_Alpha_binary_table(self, E_t, trainidxs, testidxs, C, Ctest, Ntrain, Ntest, l=1):
        # l is the index into alpha we are dealing with
        # FILTERING -- UPDATES GIVEN PREVIOUS TIMESTEPS
        # p(\pi_t | data up to and including t)
        # Variables used in smoothing step but calculated during filtering
        ...
        while n < Ntrain+Ntest:
            logging.debug("Alpha update filter: %i / %i" % (n,Ntrain+Ntest) )
            h_cov = self.h_cov(n, E_t)
...
#             logging.debug("Alpha_tilde_po " + str(alpha_tilde_po) + ", tau=" + str(tau))
        ...
        while n > 0:
            n -= 1
            logging.debug("Alpha update smoother: " + str(n) + "/" + str(Ntrain+Ntest))
            h_cov = self.h_cov(n, E_t)
            ...
            tauIdx = np.ravel_multi_index((timeIdx, agentIdx), (Ntrain+Ntest, self.K))
...
# Initialisation ---------------------------------------------------------------------------------------------------
    def __init__(self, nclasses=2, nscores=2, alpha0=None, nu0=None, K=1, uselowerbound=False, dh=None, 
                 datamodelclass=ibcc.ClassProportions, workermodelclass=DynamicConfusionMatrix):
        
        super(DynIBCC, self).__init__(nclasses, nscores, alpha0, nu0, K, uselowerbound, dh, 
                 datamodelclass, workermodelclass)
    ...
    def init_params(self, force_reset=False):
        ...
        self.workermodel.set_data_format(self.table_format_flag)
        ...
        if not self.workermodel.initialised or self.workermodel.K != self.K or self.workermodel.Tau != self.Tau or force_reset:
            self.workermodel.init_lnPi(self.K, self.Tau, self.Cknown)
    ...
    def preprocess_crowdlabels(self, crowdlabels):
        ...
        if self.table_format_flag:# crowd labels as a full KxN table? If false, use a sparse 3-column list, where 1st
            ...
            self.workermodel.Cagents = self.Cagents
            self.workermodel.Cagents_test = self.Cagents_test
            self.workermodel.Cobjects = self.Cobjects
            self.workermodel.Cobjects_test = self.Cobjects_test
    ...
    def lnjoint(self, alldata=False):
        ...
        if self.uselowerbound or alldata:
            for j in range(self.nclasses):
                ...
                for l in range(self.nscores):
                    if self.table_format_flag:
                        data += self.workermodel.lnPi[j, l, :].reshape((self.N,self.K)) * self.C[l]
                    else:
                        data[self.Cobjects, self.Cagents] += self.workermodel.lnPi[j, l, :] * self.C[l]
                self.lnpCT[:, j] = np.sum(data, 1) + self.datamodel.lnkappa[j]
            ...
            for j in range(self.nclasses):
                ...
                for l in range(self.nscores):
                    if self.table_format_flag:
                        data += self.workermodel.lnPi[j, l, self.tauidxs_test] * self.Ctest[l]
                    else:
                        data[self.Cobjects_test, self.Cagents_test] += self.workermodel.lnPi[j, l, self.tauidxs_test] * \
                                                                            self.Ctest[l]
                if not self.table_format_flag:
                    data = data[self.testidxs,:]
                self.lnpCT[self.testidxs, j] = np.sum(data, 1) + self.datamodel.lnkappa[j]
